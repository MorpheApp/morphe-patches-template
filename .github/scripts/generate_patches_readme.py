...
Usage: python3 generate_patches_readme.py <branch> [patches-list.json] [README.md]
"""
...
json_path   = Path(sys.argv[2]) if len(sys.argv) > 2 else Path("patches-list.json")
readme_path = Path(sys.argv[3]) if len(sys.argv) > 3 else Path("README.md")
...
def patches_table(patches):
    ...
    for p in sorted(patches, key=lambda x: x["name"]):
        ...
        if options:
            ...
            opts_cell = "<br>".join(f"• {p}" for p in parts)
        else:
            opts_cell = ""
...
lines = [
    f"> **[{ver}](https://github.com/MorpheApp/morphe-patches/releases/tag/{ver})**&nbsp;&nbsp;•&nbsp;&nbsp;`{branch}`&nbsp;&nbsp;•&nbsp;&nbsp;{total} patches total",
    "",
]
...
# Inject into README between marker comments
...
if START_MARKER in readme and END_MARKER in readme:
    ...
    print(generated)
    sys.stderr.write(
        f"⚠️  Markers {START_MARKER} / {END_MARKER} not found in {readme_path}. "
